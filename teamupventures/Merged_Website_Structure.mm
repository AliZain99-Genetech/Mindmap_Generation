<?xml version='1.0' encoding='utf-8'?>
<map version="1.0.1">
  <node TEXT="Home - Teamup Ventures">
    <node TEXT="Header">
      <node TEXT="Navigation Links">
        <node TEXT="Home" LINK="https://teamupventures.com/" />
        <node TEXT="The Pakistan Story" LINK="https://teamupventures.com/pakistan-story/">
          <node TEXT="Key Economic Facts">
            <node TEXT="Population: 5th largest, 225M (132.6M below 30)" />
            <node TEXT="Nominal GDP (2021): $297B" />
            <node TEXT="Net Exports (2020): $22.5B" />
            <node TEXT="Inflation: 8.2%" />
            <node TEXT="Real GDP Growth Rate (2021F): 4.7%" />
            <node TEXT="Unemployment Rate: 4.5%" />
            <node TEXT="Remittances (2021): $29.4B" />
            <node TEXT="GDP per Capita, PPP (2021F): $5230" />
          </node>
          <node TEXT="Demographic Facts">
            <node TEXT="Population Growth: 1980-2030 trend, 78M to 262M" />
            <node TEXT="GDP Growth: 1980-2030 trend, $31B to $776B" />
          </node>
          <node TEXT="Startup Ecosystem Overview">
            <node TEXT="Startup funding and deals in Pakistan (2015-2021)" />
            <node TEXT="Facts about Pakistan’s startup ecosystem">
              <node TEXT="$578M+ raised by Pakistani startups since 2015" />
              <node TEXT="$259M+ funding raised in 2021" />
              <node TEXT="700+ startups launched till date" />
              <node TEXT="83 incubators and accelerators" />
              <node TEXT="Up 28 places in World Bank’s ease of doing business (2020)" />
            </node>
          </node>
          <node TEXT="Rapidly Growing Digital Infrastructure">
            <node TEXT="Growth in internet connectivity offers startup opportunities" />
            <node TEXT="Notes on digital transformation and inclusion" />
            <node TEXT="Exponential connectivity growth: 103M broadband subscribers" />
            <node TEXT="98% of households own a mobile phone" />
          </node>
        </node>
        <node TEXT="Our Team" LINK="https://teamupventures.com/#our-team">
          <node TEXT="The Pakistan Story">
            <node TEXT="Section summary: Pakistan's digital era and investment opportunity" />
            <node TEXT="READ MORE">
              <node TEXT="Link" LINK="https://teamupventures.com/pakistan-story/" />
            </node>
          </node>
          <node TEXT="Scrapboard - Blog   News Highlights">
            <node TEXT="The Era of VCs and Boom of Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/the-era-of-vcs-and-boom-of-start-ups/" />
            </node>
            <node TEXT="To Survive Pakistan's COVID Pandemic, Grocery Stores Go Online">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/to-survive-pakistans-covid-pandemic-grocery-stores-go-online/" />
            </node>
            <node TEXT="Clicky.pk attracts $700,000 in pre-series A funding round">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/clicky-pk-attracts-700000-in-pre-series-a-funding-round/" />
            </node>
            <node TEXT="Sehat Kahani Raises $1 Million Pre-Series A for Telemedicine Network">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-sehat-kahani-raises-1-million-pre-series-a-to-grow-its-telemedicine-network/" />
            </node>
            <node TEXT="Startup Challenges: The Way Out">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/startup-challenges-the-way-out/" />
            </node>
            <node TEXT="Remotebase Raises $1.4 Million Led by Indus Valley Capital">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pak-tech-startup-remotebase-raises-1-4-million-led-by-indus-valley-capital/" />
            </node>
            <node TEXT="Shopsy Introduces Voice Search for Customers">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/islamabad-based-startup-called-shopsy-introduces-voice-search-for-its-customers/" />
            </node>
            <node TEXT="PayPro Receives a Grant of Rs 7.4 Million from USAID and SMEA">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-fintech-startup-called-paypro-receives-a-grant-of-rs-7-4-million-from-usaid-and-smea/" />
            </node>
            <node TEXT="Local Startup Using AI to Detect COVID-19">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/local-startup-is-using-ai-to-detect-covid-19/" />
            </node>
            <node TEXT="HumWell: All-Inclusive Telehealth Service">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/humwell-comes-to-rescue-with-an-all-inclusive-telehealth-service/" />
            </node>
            <node TEXT="E-commerce Marketing Tool Launched by Pakistani Entrepreneur">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-first-e-commerce-marketing-tool-launched-by-pakistani-entrepreneur-to-make-e-commerce-more-competitive/" />
            </node>
            <node TEXT="Pakistani Startups Break Records in 2020 Despite Economic Slowdown">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-startups-break-records-in-2020-despite-global-economic-slowdown/" />
            </node>
            <node TEXT="Truck It In Raises $1.5 Million Pre-Seed for Trucking Marketplace">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-truck-it-in-raises-1-5-million-pre-seed-for-its-trucking-marketplace/" />
            </node>
            <node TEXT="Edkasa Raises $320,000 Pre-Seed for Exam Prep App">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-edtech-startup-edkasa-raises-320000-pre-seed-by-launching-an-exam-prep-app-for-students/" />
            </node>
            <node TEXT="Pakistan Launches Growth Funds for Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistan-launches-growth-funds-for-startups/" />
            </node>
            <node TEXT="Bayfikr – User-Friendly Fin-Tech App for Overseas Pakistanis">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/bayfikr-a-user-friendly-fin-tech-app-making-payments-in-pakistan-easier-for-overseas-pakistanis/" />
            </node>
            <node TEXT="Fintech SadaPay Raises $7.2 Million Seed Round">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/exclusive-pakistani-fintech-sadapay-raises-7-2-million-in-countrys-largest-seed-round/" />
            </node>
            <node TEXT="Pakistani Startups Best Year Yet With Over $65 Million in Funding">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/despite-a-pandemic-pakistani-startups-had-their-best-year-yet-with-over-65-million-in-funding/" />
            </node>
            <node TEXT="NIC Welcomes Its 10th Cohort of Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/nic-welcomes-its-10th-cohort-of-startups/" />
            </node>
            <node TEXT="For a successful startup ecosystem, Pakistan needs to learn to celebrate failure">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/for-a-successful-startup-ecosystem-pakistan-needs-to-learn-to-celebrate-failure/" />
            </node>
          </node>
          <node TEXT="Contact   Careers">
            <node TEXT="Contact introduction and invitation to reach out" />
            <node TEXT="Contact Form">
              <node TEXT="Your Name*" />
              <node TEXT="Your Email*" />
              <node TEXT="Company*" />
              <node TEXT="Subject*" />
              <node TEXT="Your Message*" />
              <node TEXT="GET IN TOUCH">
                <node TEXT="Button" LINK="https://teamupventures.com/#contact-careers" />
              </node>
            </node>
          </node>
        </node>
        <node TEXT="National Incubation Centre" LINK="https://nicpakistan.pk/" />
        <node TEXT="Scrapboard" LINK="https://teamupventures.com/#scrap-board">
          <node TEXT="Blog   News Highlights">
            <node TEXT="The Era of VCs and Boom of Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/the-era-of-vcs-and-boom-of-start-ups/" />
            </node>
            <node TEXT="To Survive Pakistan #39;s COVID Pandemic, Grocery Stores Go Online">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/to-survive-pakistans-covid-pandemic-grocery-stores-go-online/" />
            </node>
            <node TEXT="Clicky.pk attracts $700,000 in pre-series A funding round">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/clicky-pk-attracts-700000-in-pre-series-a-funding-round/" />
            </node>
            <node TEXT="Sehat Kahani Raises $1 Million to Grow its Telemedicine Network">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-sehat-kahani-raises-1-million-pre-series-a-to-grow-its-telemedicine-network/" />
            </node>
            <node TEXT="Startup Challenges: The Way Out">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/startup-challenges-the-way-out/" />
            </node>
            <node TEXT="Remotebase Raises $1.4 Million Led by Indus Valley Capital">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pak-tech-startup-remotebase-raises-1-4-million-led-by-indus-valley-capital/" />
            </node>
            <node TEXT="Shopsy Introduces Voice Search for Customers">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/islamabad-based-startup-called-shopsy-introduces-voice-search-for-its-customers/" />
            </node>
            <node TEXT="PayPro Receives Rs 7.4 Million Grant from USAID and SMEA">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-fintech-startup-called-paypro-receives-a-grant-of-rs-7-4-million-from-usaid-and-smea/" />
            </node>
            <node TEXT="Startup Using AI to Detect COVID-19">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/local-startup-is-using-ai-to-detect-covid-19/" />
            </node>
            <node TEXT="HumWell Comes To Rescue with Telehealth Service">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/humwell-comes-to-rescue-with-an-all-inclusive-telehealth-service/" />
            </node>
            <node TEXT="E-commerce Marketing Tool Launched in Pakistan">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-first-e-commerce-marketing-tool-launched-by-pakistani-entrepreneur-to-make-e-commerce-more-competitive/" />
            </node>
            <node TEXT="Pakistani Startups Break Records in 2020">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-startups-break-records-in-2020-despite-global-economic-slowdown/" />
            </node>
            <node TEXT="Truck It In Raises $1.5 Million Pre-Seed for Trucking Marketplace">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-truck-it-in-raises-1-5-million-pre-seed-for-its-trucking-marketplace/" />
            </node>
            <node TEXT="Edkasa Raises $320,000 Pre-Seed for Exam Prep App">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-edtech-startup-edkasa-raises-320000-pre-seed-by-launching-an-exam-prep-app-for-students/" />
            </node>
            <node TEXT="Pakistan Launches Growth Funds for Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistan-launches-growth-funds-for-startups/" />
            </node>
            <node TEXT="Bayfikr – Fin-tech App for Overseas Pakistanis">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/bayfikr-a-user-friendly-fin-tech-app-making-payments-in-pakistan-easier-for-overseas-pakistanis/" />
            </node>
            <node TEXT="SadaPay Raises $7.2 Million in Largest Seed Round">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/exclusive-pakistani-fintech-sadapay-raises-7-2-million-in-countrys-largest-seed-round/" />
            </node>
            <node TEXT="Pakistani Startups Had Their Best Year With $65 Million Funding">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/despite-a-pandemic-pakistani-startups-had-their-best-year-yet-with-over-65-million-in-funding/" />
            </node>
            <node TEXT="NIC Welcomes Its 10th Cohort of Startups">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/nic-welcomes-its-10th-cohort-of-startups/" />
            </node>
            <node TEXT="Learning to Celebrate Failure for Startup Ecosystem">
              <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/for-a-successful-startup-ecosystem-pakistan-needs-to-learn-to-celebrate-failure/" />
            </node>
          </node>
          <node TEXT="Contact   Careers Section">
            <node TEXT="Contact   Careers Summary" />
            <node TEXT="Contact Form">
              <node TEXT="Your Name*" />
              <node TEXT="Your Email" />
              <node TEXT="Company*" />
              <node TEXT="Subject*" />
              <node TEXT="Your Message*" />
              <node TEXT="GET IN TOUCH" LINK="https://teamupventures.com/#contact-careers" />
            </node>
          </node>
        </node>
        <node TEXT="Contact   Careers" LINK="https://teamupventures.com/#contact-careers">
          <node TEXT="The Pakistan Story">
            <node TEXT="Introduction summary: Digital era, growing economy, investment opportunity" />
            <node TEXT="READ MORE">
              <node TEXT="Link: https://teamupventures.com/pakistan-story/" />
            </node>
          </node>
          <node TEXT="Scrapboard Blog   News">
            <node TEXT="The Era of VCs and Boom of Startups">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/the-era-of-vcs-and-boom-of-start-ups/" />
              </node>
            </node>
            <node TEXT="To Survive Pakistan’s COVID Pandemic, Grocery Stores Go Online">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/to-survive-pakistans-covid-pandemic-grocery-stores-go-online/" />
              </node>
            </node>
            <node TEXT="Clicky.pk attracts $700,000 in pre-series A funding round">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/clicky-pk-attracts-700000-in-pre-series-a-funding-round/" />
              </node>
            </node>
            <node TEXT="Sehat Kahani Raises $1 Million Pre-Series A to Grow its Telemedicine Network">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistans-sehat-kahani-raises-1-million-pre-series-a-to-grow-its-telemedicine-network/" />
              </node>
            </node>
            <node TEXT="Startup Challenges: The Way Out">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/startup-challenges-the-way-out/" />
              </node>
            </node>
            <node TEXT="Pak Tech Startup Remotebase Raises $1.4 Million Led by Indus Valley Capital">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pak-tech-startup-remotebase-raises-1-4-million-led-by-indus-valley-capital/" />
              </node>
            </node>
            <node TEXT="Shopsy Introduces Voice Search for its Customers">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/islamabad-based-startup-called-shopsy-introduces-voice-search-for-its-customers/" />
              </node>
            </node>
            <node TEXT="PayPro Receives a Grant of Rs 7.4 Million from USAID and SMEA">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistani-fintech-startup-called-paypro-receives-a-grant-of-rs-7-4-million-from-usaid-and-smea/" />
              </node>
            </node>
            <node TEXT="Local Startup Using AI to Detect COVID-19">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/local-startup-is-using-ai-to-detect-covid-19/" />
              </node>
            </node>
            <node TEXT="HumWell: All-inclusive Telehealth Service">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/humwell-comes-to-rescue-with-an-all-inclusive-telehealth-service/" />
              </node>
            </node>
            <node TEXT="Pakistan’s First E-commerce Marketing Tool Launched">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistans-first-e-commerce-marketing-tool-launched-by-pakistani-entrepreneur-to-make-e-commerce-more-competitive/" />
              </node>
            </node>
            <node TEXT="Pakistani Startups Break Records in 2020 Despite Slowdown">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistani-startups-break-records-in-2020-despite-global-economic-slowdown/" />
              </node>
            </node>
            <node TEXT="Truck It In Raises $1.5 Million Pre-Seed">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistans-truck-it-in-raises-1-5-million-pre-seed-for-its-trucking-marketplace/" />
              </node>
            </node>
            <node TEXT="EdTech Startup Edkasa Raises $320,000 Pre-Seed">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistani-edtech-startup-edkasa-raises-320000-pre-seed-by-launching-an-exam-prep-app-for-students/" />
              </node>
            </node>
            <node TEXT="Pakistan Launches Growth Funds for Startups">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/pakistan-launches-growth-funds-for-startups/" />
              </node>
            </node>
            <node TEXT="Bayfikr – Fin-Tech App for Overseas Pakistanis">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/bayfikr-a-user-friendly-fin-tech-app-making-payments-in-pakistan-easier-for-overseas-pakistanis/" />
              </node>
            </node>
            <node TEXT="SadaPay Raises $7.2 Million in Country’s Largest Seed Round">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/exclusive-pakistani-fintech-sadapay-raises-7-2-million-in-countrys-largest-seed-round/" />
              </node>
            </node>
            <node TEXT="Despite a Pandemic, Pakistani Startups Had Their Best Year">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/despite-a-pandemic-pakistani-startups-had-their-best-year-yet-with-over-65-million-in-funding/" />
              </node>
            </node>
            <node TEXT="NIC Welcomes its 10th Cohort of Startups">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/nic-welcomes-its-10th-cohort-of-startups/" />
              </node>
            </node>
            <node TEXT="For a Successful Startup Ecosystem, Celebrate Failure">
              <node TEXT="VIEW DETAILS">
                <node TEXT="Link: https://teamupventures.com/scrapboard/for-a-successful-startup-ecosystem-pakistan-needs-to-learn-to-celebrate-failure/" />
              </node>
            </node>
          </node>
          <node TEXT="Contact   Careers Section">
            <node TEXT="Contact prompt and invitation summary" />
            <node TEXT="Contact Form">
              <node TEXT="Your Name" />
              <node TEXT="Your Email" />
              <node TEXT="Company" />
              <node TEXT="Subject" />
              <node TEXT="Your Message" />
              <node TEXT="GET IN TOUCH" />
            </node>
          </node>
        </node>
      </node>
    </node>
    <node TEXT="Page Content">
      <node TEXT="The Pakistan Story">
        <node TEXT="Description">
          <node TEXT="Pakistan is on the verge of a new digital era. A young population, low unemployment and a growing middle class position Pakistan to become one of the most promising economies for investors in the coming decade." />
        </node>
        <node TEXT="READ MORE" LINK="https://teamupventures.com/pakistan-story/" />
      </node>
      <node TEXT="Scrapboard">
        <node TEXT="BLOG   NEWS" />
        <node TEXT="The Era of VCs and Boom of Startups">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/the-era-of-vcs-and-boom-of-start-ups/" />
        </node>
        <node TEXT="To Survive Pakistan's COVID Pandemic, Grocery Stores Go Online">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/to-survive-pakistans-covid-pandemic-grocery-stores-go-online/" />
        </node>
        <node TEXT="Clicky.pk attracts $700,000 in pre-series A funding round">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/clicky-pk-attracts-700000-in-pre-series-a-funding-round/" />
        </node>
        <node TEXT="Pakistan's 'Sehat Kahani' Raises $1 Million Pre-Series A to Grow its Telemedicine Network">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-sehat-kahani-raises-1-million-pre-series-a-to-grow-its-telemedicine-network/" />
        </node>
        <node TEXT="Startup Challenges: The Way Out">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/startup-challenges-the-way-out/" />
        </node>
        <node TEXT="Pak Tech Startup 'Remotebase' Raises $1.4 Million Led by Indus Valley Capital">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pak-tech-startup-remotebase-raises-1-4-million-led-by-indus-valley-capital/" />
        </node>
        <node TEXT="Islamabad-based Startup Called 'Shopsy' Introduces Voice Search for its Customers">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/islamabad-based-startup-called-shopsy-introduces-voice-search-for-its-customers/" />
        </node>
        <node TEXT="Pakistani Fintech Startup Called 'PayPro' Receives a Grant of Rs 7.4 Million from USAID and SMEA">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-fintech-startup-called-paypro-receives-a-grant-of-rs-7-4-million-from-usaid-and-smea/" />
        </node>
        <node TEXT="Local Startup is Using AI to Detect COVID-19">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/local-startup-is-using-ai-to-detect-covid-19/" />
        </node>
        <node TEXT="HumWell Comes To Rescue, With An All-Inclusive Telehealth Service">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/humwell-comes-to-rescue-with-an-all-inclusive-telehealth-service/" />
        </node>
        <node TEXT="Pakistan's First E-commerce Marketing Tool Launched by Pakistani Entrepreneur to Make E-commerce More Competitive">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-first-e-commerce-marketing-tool-launched-by-pakistani-entrepreneur-to-make-e-commerce-more-competitive/" />
        </node>
        <node TEXT="Pakistani Startups Break Records in 2020 Despite Global Economic Slowdown">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-startups-break-records-in-2020-despite-global-economic-slowdown/" />
        </node>
        <node TEXT="Pakistan's 'Truck It In' Raises $1.5 Million Pre-Seed for its Trucking Marketplace">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistans-truck-it-in-raises-1-5-million-pre-seed-for-its-trucking-marketplace/" />
        </node>
        <node TEXT="Pakistani EdTech Startup 'Edkasa' Raises $320,000 Pre-Seed by Launching an Exam Prep App for Students">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistani-edtech-startup-edkasa-raises-320000-pre-seed-by-launching-an-exam-prep-app-for-students/" />
        </node>
        <node TEXT="Pakistan Launches Growth Funds for Startups">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/pakistan-launches-growth-funds-for-startups/" />
        </node>
        <node TEXT="Bayfikr – a User-Friendly Fin-Tech app, Making Payments in Pakistan Easier for Overseas Pakistanis">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/bayfikr-a-user-friendly-fin-tech-app-making-payments-in-pakistan-easier-for-overseas-pakistanis/" />
        </node>
        <node TEXT="Pakistani Fintech SadaPay Raises $7.2 Million in Country's Largest Seed Round">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/exclusive-pakistani-fintech-sadapay-raises-7-2-million-in-countrys-largest-seed-round/" />
        </node>
        <node TEXT="Despite a Pandemic, Pakistani Startups Had Their Best Year Yet With Over $65 Million in Funding">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/despite-a-pandemic-pakistani-startups-had-their-best-year-yet-with-over-65-million-in-funding/" />
        </node>
        <node TEXT="NIC Welcomes It's 10th Cohort of Startups">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/nic-welcomes-its-10th-cohort-of-startups/" />
        </node>
        <node TEXT="For a successful startup ecosystem, Pakistan needs to learn to celebrate failure">
          <node TEXT="View Details" LINK="https://teamupventures.com/scrapboard/for-a-successful-startup-ecosystem-pakistan-needs-to-learn-to-celebrate-failure/" />
        </node>
      </node>
      <node TEXT="Contact   Careers" LINK="https://teamupventures.com/#contact-careers">
        <node TEXT="Contact Form">
          <node TEXT="Your Name" />
          <node TEXT="Your Email" />
          <node TEXT="Company" />
          <node TEXT="Subject" />
          <node TEXT="Your Message" />
          <node TEXT="GET IN TOUCH" LINK="https://teamupventures.com/#contact-careers" />
        </node>
      </node>
    </node>
    <node TEXT="Footer">
      <node TEXT="Company Info">
        <node TEXT="Teamup Ventures is licensed and regulated by the Dubai Financial Services Authority. Company Number: 4684" />
      </node>
      <node TEXT="Dubai Office">
        <node TEXT="Teamup Ventures" />
        <node TEXT="Level 1 Gate Avenue – South" LINK="https://maps.app.goo.gl/6TW76qnbMrejyLBt6" />
        <node TEXT="Dubai International Financial Centre (DIFC)" LINK="https://maps.app.goo.gl/yuURWpV31zyayDeN7" />
        <node TEXT="PO Box 50 70 33" />
        <node TEXT="Dubai, UAE" />
      </node>
      <node TEXT="Other Contact">
        <node TEXT="contact@teamupventures.com" LINK="mailto:contact@teamupventures.com" />
        <node TEXT="Social Media">
          <node TEXT="Twitter" LINK="https://twitter.com/VenturesTeamup" />
          <node TEXT="Instagram" LINK="https://www.instagram.com/teamupventures/" />
          <node TEXT="LinkedIn" LINK="https://www.linkedin.com/company/teamup-ventures/" />
        </node>
        <node TEXT="Terms   Conditions" LINK="https://teamupventures.com/terms-conditions/" />
      </node>
    </node>
  </node>
</map>